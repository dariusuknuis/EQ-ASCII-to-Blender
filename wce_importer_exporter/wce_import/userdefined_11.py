...
def create_node_group_ud11():
    # Create the node group
    node_group = bpy.data.node_groups.new(name="USERDEFINED_11", type='ShaderNodeTree')
    ...
    principled_node.inputs['Alpha'].default_value = 0.25  # Set alpha to 0.25 for 75% transparency
    ...
    group_links.new(passable.outputs['Result'], principled_node.inputs['Base Color'])
    group_links.new(passable.outputs['Value'],  mix_shader.inputs['Fac'])
    group_links.new(passable.outputs['BSDF'],   mix_shader.inputs[2])
    group_links.new(attr_node.outputs['Vector'], principled_node.inputs['Normal'])
    group_links.new(principled_node.outputs['BSDF'], mix_shader.inputs[1])
    group_links.new(mix_shader.outputs['Shader'], out_sock['Shader'])

    return node_group

def create_material_with_node_group_ud11(material_name, texture_path, node_group):
