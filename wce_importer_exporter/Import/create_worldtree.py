...
def create_bsp_plane_mesh(name="BSPPlaneMesh", size=DEFAULT_SIZE):
    ...
    """
    half = size / 2.0
    # Start with a default plane in the XY plane (face normal = (0,0,1)):
    verts = [
        (-half, -half, 0),
        ( half, -half, 0),
        ( half,  half, 0),
    ...
    faces = [(0, 1, 2, 3)]
    # Rotate the mesh by -90° about the X axis to move the face into the XZ plane.
...
def calculate_point_on_plane(normal, d):
    ...
    This function returns a point on the plane using a simple heuristic.
    """
    a, b, c = normal
    if abs(c) > 1e-6:
...
def compute_scale_from_location(location, default_size=DEFAULT_SIZE):
    ...
    The idea is to increase the plane's size by the absolute value of the highest coordinate.
    """
...
def create_worldtree(worldtree_data):
    ...
    Each node is represented by a plane mesh (with a translucent yellow material) that is 
    ...
    # Create a root empty for organizational purposes.
    root_obj = bpy.data.objects.new("WorldTree_Root", None)
    bpy.context.collection.objects.link(root_obj)

    # Create a base plane mesh (shared among nodes).
    ...
    for node in worldtree_data["nodes"]:
        ...
        # Compute the rotation so that the local +Y of the plane (which, thanks to our mesh, is the face normal) aligns with the node's normal.
        rotation = rotation_from_normal(normal)
        # Compute a scale factor based on the maximum offset of the position from the origin.
        scale_factor = compute_scale_from_location(position, default_size=DEFAULT_SIZE)

        # Create a new object using the base mesh.
        node_obj = bpy.data.objects.new(node_name, base_mesh)
        node_obj.location = position
        node_obj.rotation_euler = rotation
        node_obj.scale = (scale_factor, scale_factor, scale_factor)
        # Assign the material.
        ...
        # Add a Solidify modifier with thickness 3.0 and offset 0.0.
    ...
    # Re-parent child nodes while preserving world transforms (simulate "Keep Transform").
    for node in worldtree_data["nodes"]:
    ...
    print(f"Created WorldTree with {len(worldtree_data['nodes'])} nodes as plane meshes.")
    return root_obj
