...
def cleanup_bmesh(bm, deg_edge_dist=1e-3, angle_limit=radians(5), use_dissolve_boundaries=False, delimit={'NORMAL'}):
    """
    1) Dissolve any edge shorter than deg_edge_dist.
    2) BMesh-based equivalent of Blender's limited dissolve operator.
    """
    ...
    # Tag elements like Blender's op does — only keep selected geometry
    geom_edges = [e for e in bm.edges]
    geom_verts = [v for v in bm.verts]

    bmesh.ops.dissolve_limit(
        bm,
        angle_limit=angle_limit,
        use_dissolve_boundaries=use_dissolve_boundaries,
        delimit=delimit,  # <-- Keep as set of strings
        edges=geom_edges,
        verts=geom_verts,
    )
